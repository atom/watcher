...
const {RegistryNode} = require('./registry-node')
const {RegistryWatcherNode} = require('./registry-watcher-node')
  ...
  add (pathSegments, attachToNative) {
    ...
    const attachToNew = (childPaths) => {
      ...
      const sub = native.onWillStop(split => {
        ...
        this.root = this.root.remove(pathSegments, createNative) || new RegistryNode()
      })
